...
Four prediction tasks are supported (choose with ``task=``), each turning a
different clinical column into the label and using the other 12 columns as
features:
  ...
  * ``"disease"`` (5-class, the default) -- predict the presence and severity
    ...
    variables (the only task that keeps every column as a feature, since its
    own label is not one of the 13).
  * ``"cp"`` (4-class) -- predict the *chest-pain type* (1 typical angina /
    2 atypical angina / 3 non-anginal pain / 4 asymptomatic).
  * ``"restecg"`` (3-class) -- predict the *resting ECG result* (0 normal /
    1 ST-T wave abnormality / 2 probable-or-definite left ventricular
    hypertrophy). Essentially complete (2 of 920 patients missing).
  * ``"exang"`` (binary) -- predict *exercise-induced angina* (0 no / 1 yes).
    The one binary secondary task, so ``federated.make_model`` picks
    ``LogisticModel`` instead of ``SoftmaxModel`` for it automatically.

Two of the clinical columns -- ``ca`` and ``thal`` -- are deliberately not
offered as prediction targets: they are missing for 80-99% of patients outside
Cleveland (see ``heterogeneity.missingness_report``), which would leave those
sites almost unlabeled rather than merely federated. ``slope`` has the same
problem in a milder, more site-skewed form (0% missing at Cleveland, 65% at
Hungary, 14% at Switzerland, 51% at V.A.) and is excluded for the same reason.
``preprocess`` drops any row whose label for the chosen task is itself missing,
so a task built on a column with real missingness silently trains and
evaluates on a smaller, per-site-skewed population -- correct behavior, but
worth knowing about before picking a task.
"""
...
# input features. Every secondary task uses the other 12 clinical variables and
# does NOT use its own column as a feature (predicting a field from itself is
# not a task).

CP_FEATURES = [c for c in FEATURES if c != "cp"]
RESTECG_FEATURES = [c for c in FEATURES if c != "restecg"]
EXANG_FEATURES = [c for c in FEATURES if c != "exang"]
...
TASKS = {
    ...
    "restecg": {
        "label_col": "restecg",
        "features": RESTECG_FEATURES,
        "class_names": ["normal", "ST-T abnormality", "LV hypertrophy"],
        # restecg is already 0-2; only 2 patients (of 920) have it missing
        "encode": lambda s: np.asarray(s).astype(int),
    },
    "exang": {
        "label_col": "exang",
        "features": EXANG_FEATURES,
        "class_names": ["no angina", "exercise-induced angina"],
        # exang is already 0/1; a binary task, so make_model picks LogisticModel
        "encode": lambda s: np.asarray(s).astype(int),
    },
}
...
def preprocess(
    ...
    ----------
    task:
        one of the keys in ``TASKS`` (see module docstring).
    standardize:
        z-score each feature. Pass ``scaler_stats=(mean, std)`` to apply shared
        statistics (so every site uses the same transform).

    Rows whose label is itself missing (irrelevant for ``disease``/``cp``, which
    have none, but real for a handful of other clinical fields -- e.g. ``slope``
    is unrecorded for site fractions ranging from 0% at Cleveland to 65% at
    Hungary) are dropped before feature extraction: a patient with no target
    cannot be trained on or scored, no matter how complete their features are.
    """
    cfg = _task_config(task)
    feats = cfg["features"]
    df = df[df[cfg["label_col"]].notna()]
